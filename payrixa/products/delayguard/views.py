...
from payrixa.products.delayguard import DELAYGUARD_V1_SIGNAL_TYPE
...
class DelayGuardDashboardView(LoginRequiredMixin, ProductEnabledMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            signals_qs = base_queryset.order_by('-created_at')[:50]
            ...
            total_at_risk = base_queryset.aggregate(
                total=Sum('estimated_dollars_at_risk')
            )['total'] or 0
    ...
    def _get_recovery_stats(self, customer):
        ...
        recent_recoveries = OperatorJudgment.objects.filter(
            customer=customer,
            recovered_amount__isnull=False
        ).select_related('alert_event', 'operator').order_by('-recovered_date')[:5]
    ...
    def _check_similar_alerts(self, customer, signal):
        ...
        similar_signals = PaymentDelaySignal.objects.filter(
            customer=customer,
            payer=signal.payer,
            signal_type=signal.signal_type
        ).exclude(id=signal.id).order_by('-created_at')[:10]
        ...
        similar_judgments = OperatorJudgment.objects.filter(
            alert_event_id__in=similar_alert_ids,
            customer=customer
        ).order_by('-created_at')
