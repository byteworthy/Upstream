...
class CeleryTaskTests(TestCase):
    ...
    def test_drift_detection_task_structure(self):
        ...
        from payrixa.tasks import run_drift_detection_task
        
        self.assertEqual(
            run_drift_detection_task.name,
            'payrixa.tasks.run_drift_detection'
        )

    def test_alert_task_structure(self):
        """Test alert task has correct structure."""
        from payrixa.tasks import send_alert_task
        
        self.assertEqual(
            send_alert_task.name,
            'payrixa.tasks.send_alert'
        )

    def test_webhook_task_structure(self):
        """Test webhook task has correct structure."""
        from payrixa.tasks import send_webhook_task
        
        self.assertEqual(
            send_webhook_task.name,
            'payrixa.tasks.send_webhook'
        )

    def test_report_artifact_task_structure(self):
        """Test report artifact generation task has correct structure."""
        from payrixa.tasks import generate_report_artifact_task
        
        self.assertEqual(
            generate_report_artifact_task.name,
            'payrixa.tasks.generate_report_artifact'
        )

    def test_scheduled_report_task_structure(self):
        """Test scheduled report task has correct structure."""
        from payrixa.tasks import send_scheduled_report_task
        
        self.assertEqual(
            send_scheduled_report_task.name,
            'payrixa.tasks.send_scheduled_report'
        )

    def test_tasks_are_registered(self):
        """Test that tasks are properly registered with Celery."""
        from payrixa.celery import app
        
        registered_tasks = list(app.tasks.keys())
        
        # Check that our tasks are registered
        self.assertIn('payrixa.tasks.run_drift_detection', registered_tasks)
        self.assertIn('payrixa.tasks.send_alert', registered_tasks)
        self.assertIn('payrixa.tasks.send_webhook', registered_tasks)
        self.assertIn('payrixa.tasks.generate_report_artifact', registered_tasks)
        self.assertIn('payrixa.tasks.send_scheduled_report', registered_tasks)
...
class CeleryConfigurationTests(TestCase):
    ...
    def test_celery_app_exists(self):
        ...
        self.assertEqual(app.main, 'payrixa')
    ...
    def test_celery_autodiscover_works(self):
        ...
        task_names = [name for name in app.tasks.keys() if name.startswith('payrixa')]
