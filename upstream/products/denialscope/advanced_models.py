...
class DenialCluster(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="denial_clusters"
    )
    ...
    cluster_name = models.CharField(
        max_length=200, help_text="Human-readable cluster name"
    )
    ...
    # Payer/CPT patterns
    payers = models.JSONField(default=list, help_text="List of payers in this cluster")
    ...
    total_denied_dollars = models.DecimalField(
    ...
    silhouette_score = models.FloatField(
        blank=True, null=True, help_text="ML silhouette score for cluster quality"
    )
    ...
    resolution_strategy = models.TextField(
        blank=True, help_text="Recommended resolution strategy for this cluster"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(cluster_confidence__gte=0.0)
                & models.Q(cluster_confidence__lte=1.0),
...
class DenialCascade(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="denial_cascades"
    )
    ...
    claim_ids = models.JSONField(
        default=list, help_text="List of ClaimRecord IDs in this cascade"
    )
    claim_count = models.IntegerField(default=0)
    ...
    # Pattern analysis
    pattern_summary = models.TextField(help_text="Description of the cascade pattern")
...
class PreDenialWarning(BaseModel):
    ...
    risk_factors = models.JSONField(
        default=list, help_text="List of identified risk factors: [{factor, weight}]"
    )
    ...
    intervention_deadline = models.DateField(
        null=True, blank=True, help_text="Deadline for intervention"
    )
    ...
    model_version = models.CharField(
        max_length=50, default="v1.0", help_text="ML model version used for prediction"
    )
    prediction_features = models.JSONField(
        default=dict, help_text="Features used in prediction (for explainability)"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(denial_probability__gte=0.0)
                & models.Q(denial_probability__lte=1.0),
    ...
    def __str__(self):
        prob = self.denial_probability
        return f"{self.warning_id}: {self.warning_type} - {prob:.0%} probability"
...
class AppealTemplate(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="appeal_templates"
    )
    ...
    cpt_codes = models.JSONField(
        default=list, blank=True, help_text="CPT codes this template applies to"
    )
    ...
    success_count = models.IntegerField(
        default=0, help_text="Number of successful appeals using this template"
    )
    ...
    generation_confidence = models.FloatField(
        validators=[MinValueValidator(0.0), MaxValueValidator(1.0)], default=0.8
    )
...
class AppealGeneration(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="appeal_generations"
    )
    ...
    # Related entities
    claim_record = models.ForeignKey(
        "upstream.ClaimRecord", on_delete=models.CASCADE, related_name="appeals"
    )
    ...
    # Generated content
    appeal_letter = models.TextField(help_text="Generated appeal letter content")
    required_documentation = models.JSONField(
        default=list, help_text="List of required supporting documents"
    )
    ...
    appeal_reason = models.TextField(help_text="Reason/argument for appeal")
    ...
    edited_by_user = models.BooleanField(
        default=False, help_text="Whether user manually edited the generated appeal"
    )
