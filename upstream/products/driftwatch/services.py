...
class DriftWatchSignalService:
    ...
    def compute_all_signals(
        self, report_run: ReportRun, start_date=None, end_date=None
    ) -> Dict:
        """
    ...
    def detect_denial_rate_drift(
        ...
        for key, current_data in current_stats.items():
            ...
            if (
                baseline_data["total"] < self.MIN_SAMPLE_SIZE
                or current_data["total"] < self.MIN_SAMPLE_SIZE
            ):
                continue
            ...
            if delta >= self.DENIAL_RATE_THRESHOLD:
                ...
                confidence, p_value = self._calculate_proportion_confidence(
                    baseline_data["denials"],
                    baseline_data["total"],
                    current_data["denials"],
                    current_data["total"],
                )
    ...
    def detect_underpayment_variance(
        ...
        for key, current_data in current_stats.items():
            ...
            # Require minimum sample size
            if (
                baseline_data["count"] < self.MIN_SAMPLE_SIZE
                or current_data["count"] < self.MIN_SAMPLE_SIZE
            ):
                continue
            ...
            if variance >= self.UNDERPAYMENT_THRESHOLD:
                ...
                confidence = self._calculate_continuous_confidence(
                    baseline_data["count"],
                    baseline_data.get("std_dev", 0),
                    current_data["count"],
                    current_data.get("std_dev", 0),
                )
    ...
    def detect_payment_delay(
        ...
        for key, current_data in current_stats.items():
            ...
            if delta_days >= self.PAYMENT_DELAY_THRESHOLD:
                ...
                cash_flow_impact = (
                    (delta_days / 365) * avg_payment * current_data["count"]
                )
    ...
    def detect_auth_failure_spike(
        ...
        for key, current_data in current_stats.items():
            ...
            if delta >= self.AUTH_FAILURE_THRESHOLD:
                confidence, p_value = self._calculate_proportion_confidence(
                    baseline_data["auth_failures"],
                    baseline_data["total"],
                    current_data["auth_failures"],
                    current_data["total"],
                )
    ...
    def detect_approval_rate_decline(
        ...
        for key, current_data in current_stats.items():
            ...
            if delta >= 0.05:  # 5% decline
                confidence, p_value = self._calculate_proportion_confidence(
                    baseline_data["approved"],
                    baseline_data["total"],
                    current_data["approved"],
                    current_data["total"],
                )
    ...
    def detect_processing_time_drift(
        ...
        for key, current_data in current_stats.items():
            ...
            # Alert if processing time increased by threshold
            if delta_days >= PROCESSING_TIME_DRIFT_THRESHOLD_DAYS:
                confidence = self._calculate_continuous_confidence(
                    baseline_data["count"],
                    baseline_data.get("std_dev", 0),
                    current_data["count"],
                ...
                severity = self._calculate_severity(
                    delta_days / PROCESSING_TIME_DRIFT_NORMALIZATION_DIVISOR,
                    PROCESSING_TIME_DRIFT_SEVERITY_THRESHOLD,
                )
    ...
    def _get_payment_timing_stats(self, start_date, end_date) -> Dict:
        ...
        claims = (
            ...
            .values("payer", "cpt_group")
            .annotate(count=Count("id"), avg_payment=Avg("paid_amount"))
        )
    ...
    def _get_auth_denial_stats(self, start_date, end_date) -> Dict:
        ...
        claims = (
            ...
            .annotate(
                ...
                auth_failures=Count(
                    "id",
                    filter=Q(outcome="DENIED", denial_reason_code__icontains="auth")
                    | Q(
                        outcome="DENIED", denial_reason_text__icontains="authorization"
                    ),
                ),
                avg_allowed=Avg("allowed_amount"),
            )
    ...
    def _get_approval_stats(self, start_date, end_date) -> Dict:
        ...
        claims = (
            ...
            .values("payer", "cpt_group")
            .annotate(total=Count("id"), approved=Count("id", filter=Q(outcome="PAID")))
        )
    ...
    def _calculate_proportion_confidence(
        ...
        # Pooled proportion
        p_pool = (baseline_successes + current_successes) / (
    ...
    def _publish_drift_event(self, signal: DriftEvent, event_title: str):
        ...
        publish_event(
            ...
            payload={
                ...
                "summary": (
                    f"{signal.payer} - {signal.cpt_group}: {signal.drift_type} "
                    f"changed from {signal.baseline_value:.2f} to "
                    f"{signal.current_value:.2f}"
                ),
            },
        )
