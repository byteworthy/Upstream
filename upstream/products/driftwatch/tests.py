...
No new models - reuses upstream.models.DriftEvent via payer_drift service.
"""
...
class DriftWatchTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="driftwatch_user", password="pass"
        )
        UserProfile.objects.create(user=self.user, customer=self.customer, role="admin")
        self.client.force_login(self.user)

        ProductConfig.objects.create(
            customer=self.customer, product_slug="driftwatch", enabled=True
        )

        self.upload = Upload.all_objects.create(
            customer=self.customer, filename="drift_claims.csv", status="success"
        )

    def _create_claim(
        self, payer, outcome, days_ago, cpt_group="OFFICE", allowed_amount=100
    ):
    ...
    def test_v1_signal_volume_spike_fires(self):
        ...
        with customer_context(self.customer):
            report_run = compute_weekly_payer_drift(
                customer=self.customer, baseline_days=90, current_days=14, min_volume=30
            )
            ...
            drift_event_count = DriftEvent.objects.filter(
                customer=self.customer
            ).count()
            ...
            denial_rate_event = DriftEvent.objects.filter(
                customer=self.customer, drift_type="DENIAL_RATE"
            ).first()
    ...
    def test_no_new_models_created(self):
        ...
        report_run = ReportRun.all_objects.create(
            customer=self.customer, run_type="weekly", status="success"
        )
    ...
    def test_generate_driftwatch_demo_creates_events(self):
        ...
        out = StringIO()
        call_command(
            "generate_driftwatch_demo", "--customer", self.customer.id, stdout=out
        )
