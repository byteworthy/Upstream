...
def get_alert_interpretation(evidence_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    is_likely_noise = (
        severity_value < SEVERITY_THRESHOLD_LOW
        and abs(delta_value) < ALERT_MEDIUM_URGENCY_DELTA
    )
...
def _normalize_severity(severity) -> float:
    ...
    if isinstance(severity, str):
        return SEVERITY_NUMERIC_VALUES.get(severity.lower(), SEVERITY_DEFAULT)
    try:
...
def _calculate_urgency(severity_value: float, delta_value: float) -> Dict[str, str]:
    ...
    # High urgency: high severity OR large delta
    if (
    ...
    # Medium urgency: medium severity OR moderate delta
    if (
...
def _generate_action_steps(
    urgency_level: str, signal_type: str, entity_label: str
) -> List[str]:
...
def build_denialscope_evidence_payload(
    ...
    if aggregates is not None:
        evidence_rows = list(
            aggregates.values("payer", "denial_reason")
            .annotate(
        ...
        for row in evidence_rows:
            ...
            denial_rate = (
                (row["denied_count"] / total_submitted) if total_submitted > 0 else 0
            )
    ...
    payload = {
        ...
        "one_sentence_explanation": signal.summary_text
        if signal
        else "No DenialScope signals yet.",
