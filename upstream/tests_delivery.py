...
class AlertDeliveryTests(TestCase):
    ...
    def test_duplicate_alert_event_prevention(self):
        """Test duplicate AlertEvents not created for same event."""
        with customer_context(self.customer):
    ...
    def test_alert_sent_via_console_backend(self):
        ...
        # Check PDF attachment (may be absent if WeasyPrint fails)
        # Code logs error and continues without attachment
    ...
    def test_failed_send_updates_status(self):
        ...
        with patch(
            "upstream.alerts.services.EmailMultiAlternatives.send",
    ...
    def test_suppression_window_prevents_duplicate_sends(self):
        ...
        with customer_context(self.customer):
            ...
            self.assertEqual(
                alert_event2.error_message, "suppressed"
            )  # Suppression marker
    ...
    def test_pdf_artifact_not_duplicated_on_resend(self):
        ...
        # Send first time (may fail due to WeasyPrint issue)
        send_alert_notification(alert_event)
...
class WebhookDeliveryTests(TestCase):
    ...
    @patch("upstream.services.webhook_processor.requests.post")
